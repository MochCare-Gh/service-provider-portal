...
<script stype="text/javascript">
    ...
    $('#Stafftypes-table').Delete({Type:'DELETE',Headers:{'X-CSRF-TOKEN':'<?php echo csrf_token();?>'}, ModuleName:'Stafftypes', ModuleItemName:'StafftypesItem', NgAppName:'ngStafftypesApp'});
    });
</script>
@stop
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                <div class="x_title">
                    <div class="row">
                       <div class="col-md-8 col-sm-8 col-xs-7"><h2>Stafftypes's List</h2></div>
                       <div class="col-md-4 col-sm-4 col-xs-5">
                           <a class="btn btn-primary form-modal-button pull-right" href="{{ route('Stafftypesadd') }}">Add New Stafftypes</a>
                       </div>
                    </div>
                    <div class="clearfix"></div>
                </div>
                <div class="x_content">
                        <table  ng-app="ngStafftypesApp" ng-controller="ngStafftypesAppcontroller" class="table table-striped responsive-utilities jambo_table dataTable" id="Stafftypes-table">
                            <thead>
                                <tr>
...
<script type="text/javascript">
            ...
            var SelectedCheckboxes = function() { return $('input:checkbox:checked.Stafftypes_record').map(function () { return this.value; }).get(); }
            $.ajaxSetup({
                headers: { 'X-CSRF-TOKEN':'{{ csrf_token() }}','X-Requested-With': 'XMLHttpRequest'}
            });
            ListTable = $('#Stafftypes-table').DataTable({    
                    ...
                    buttons: ['copy', 'csv', 'excel', 'pdf', 'print','colvis',
                        {  text: 'Delete',
                           action: function ( e, dt, node, config ) {   
                                ...
                                if (TrashItem) {ajaxAction("{!! route('Stafftypesdeletemultiple') !!}",'DELETE');}
                        }
                        }
                    ],
                    order: [[1, 'asc']],
            ...
            $('body').on('ifToggled','#check-all', function (event) {
                    if($(this).is(':checked')){$('input.Stafftypes_record').iCheck('check'); } else	       { $('input.Stafftypes_record').iCheck('uncheck');}
            });
            });</script>
