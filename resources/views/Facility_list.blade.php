...
<script stype="text/javascript">
    ...
    $('#Facility-table').Delete({Type:'DELETE',Headers:{'X-CSRF-TOKEN':'<?php echo csrf_token();?>'}, ModuleName:'Facility', ModuleItemName:'FacilityItem', NgAppName:'ngFacilityApp'});
    });
</script>
@stop
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                <div class="x_title">
                    <div class="row">
                       <div class="col-md-8 col-sm-8 col-xs-7"><h2>Facility's List</h2></div>
                       <div class="col-md-4 col-sm-4 col-xs-5">
                           <a class="btn btn-primary form-modal-button pull-right" href="{{ route('Facilityadd') }}">Add New Facility</a>
                       </div>
                    </div>
                    <div class="clearfix"></div>
                </div>
                <div class="x_content">
                        <table  ng-app="ngFacilityApp" ng-controller="ngFacilityAppcontroller" class="table table-striped responsive-utilities jambo_table dataTable" id="Facility-table">
                            <thead>
                                <tr>
...
<script type="text/javascript">
            ...
            var SelectedCheckboxes = function() { return $('input:checkbox:checked.Facility_record').map(function () { return this.value; }).get(); }
            $.ajaxSetup({
                headers: { 'X-CSRF-TOKEN':'{{ csrf_token() }}','X-Requested-With': 'XMLHttpRequest'}
            });
            ListTable = $('#Facility-table').DataTable({    
                    ...
                    buttons: ['copy', 'csv', 'excel', 'pdf', 'print','colvis',
                        {  text: 'Delete',
                           action: function ( e, dt, node, config ) {   
                                ...
                                if (TrashItem) {ajaxAction("{!! route('Facilitydeletemultiple') !!}",'DELETE');}
                        }
                        }
                    ],
                    order: [[1, 'asc']],
            ...
            $('body').on('ifToggled','#check-all', function (event) {
                    if($(this).is(':checked')){$('input.Facility_record').iCheck('check'); } else	       { $('input.Facility_record').iCheck('uncheck');}
            });
            });</script>
